...
do_install_append() {
    # Remove unversioned pkg-config files due to conflict with ImageMagick 7.
    # Versioned files with -6.Q16 suffix are still available.
    for pkg in ImageMagick ImageMagick++ MagickCore Magick++ MagickWand Wand; do
        rm ${D}${libdir}/pkgconfig/$pkg.pc
    done
}
